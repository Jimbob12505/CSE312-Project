...
export default function Game() {
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        console.error("Failed to load avatar in game:", error);
      }
    };
  ...
  const drawWorldBorders = (ctx: CanvasRenderingContext2D) => {
    ...
    if (camera.y + canvas.height >= WORLD_HEIGHT - BORDER_THICKNESS) {
      ...
      ctx.fillRect(
        ...
        y,
        canvas.width,
        BORDER_THICKNESS + (canvas.height - (WORLD_HEIGHT - camera.y))
      );
    }
    ...
    if (camera.x + canvas.width >= WORLD_WIDTH - BORDER_THICKNESS) {
      ...
      ctx.fillRect(
        x,
        0,
        BORDER_THICKNESS + (canvas.width - (WORLD_WIDTH - camera.x)),
        canvas.height
      );
    }
  };
  ...
  const updateCamera = (head: Point) => {
    ...
    cameraRef.current.x = Math.max(
      0,
      Math.min(WORLD_WIDTH - viewportWidth, cameraRef.current.x)
    );
    cameraRef.current.y = Math.max(
      0,
      Math.min(WORLD_HEIGHT - viewportHeight, cameraRef.current.y)
    );
  };
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      if (segmentsRef.current.length > 0) {
        ...
        cameraRef.current.x = Math.max(
          0,
          Math.min(WORLD_WIDTH - canvas.width, cameraRef.current.x)
        );
        cameraRef.current.y = Math.max(
          0,
          Math.min(WORLD_HEIGHT - canvas.height, cameraRef.current.y)
        );
      }
    };
    ...
    const animate = (currentTime: number) => {
      ...
      if (!aliveRef.current) {
        ...
        if (deathTimeRef.current) {
          const respawnTime = Math.max(
            0,
            3 - Math.floor((Date.now() - deathTimeRef.current) / 1000)
          );
      ...
      for (const otherSnake of otherPlayersRef.current) {
        ...
        if (playerIdRef.current && otherSnake.id === playerIdRef.current)
          continue;
        ...
        if (idx >= 0) {
          ...
          if (ourHead && theirHead) {
            const distance = Math.hypot(
              ourHead.x - theirHead.x,
              ourHead.y - theirHead.y
            );
